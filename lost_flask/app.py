# app.py (Scraping feature removed)
...
# ▼▼▼ The /add_from_syllabus route has been removed. ▼▼▼
...
def add_review(id):
    ...
    if not all([rating, attendance, test, report]):
        flash('評価、出欠、テスト、レポートの項目はすべて入力してください。', 'error')
        return redirect(url_for('course_detail', id=id))
    new_review = Review(
        review=review_text, rating=int(rating), attendance=attendance,
        test=test, report=report, course_id=course.id, author=current_user
    )
